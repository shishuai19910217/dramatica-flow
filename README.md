...
<p align="center">
  ...
  &nbsp;
  <img src="https://img.shields.io/badge/模型-DeepSeek%2B%20Ollama-4CAF50?style=for-the-badge" alt="LLM Support"/>
</p>
...
系统将小说创作抽象为可量化、可追踪、可审计的工程流程，通过 **因果链管理、情感弧线追踪、伏笔系统、关系网络、多线叙事、信息边界**等核心机制，确保 AI 生成的内容具有真正的叙事逻辑和内在一致性，而非松散的事件堆砌。

### 与普通 AI 写作工具的核心差异

| 维度 | 普通 AI 写作工具 | **Dramatica-Flow** |
|------|----------------|---------------------|
| 叙事逻辑 | 逐段生成，缺乏全局因果 | **强制建模因果链**：每个事件必须回答 "为什么 → 发生了什么 → 导致了什么" |
| 角色一致性 | 容易 OOC（性格崩塌） | **信息边界系统**：角色只知道亲眼所见/亲耳所闻，杜绝全知视角污染 |
| 长篇连贯性 | 前后矛盾频发 | **世界状态快照 + 真相文件**：章节间状态累积，永不丢失 |
| 伏笔管理 | 无 | **伏笔生命周期**：埋设 → 追踪 → 预警 → 回收，超期未回填自动提醒 |
| 质量控制 | 无审计 | **三层审计机制**：规则验证 → 叙事审计 → 修订闭环（最多 2 轮） |
...
| 类型 | 说明 | 示例 |
|------|------|------|
| **Foreshadow**（伏笔） | 暗线铺垫 | 第 3 章提及的神秘玉佩，第 28 章揭示身世 |
| **Promise**（承诺） | 对读者的叙事承诺 | "三年之约" 必须在三年内兑现 |
| **Mystery**（悬念） | 未解之谜 | 密室中消失的灵力到底去了哪里 |
| **Conflict**（冲突） | 未解决的矛盾 | 两大势力的暗战何时爆发 |

系统自动追踪伏笔状态，超期未回收时发出预警，杜绝 "挖坑不填"。

### 3. 情感弧线 — 可视化的角色成长

为每个角色建立 1-10 级的情感强度追踪，支持 Dramatica 双层需求模型：**外部目标**（可见的、可量化的）与 **内在渴望**（角色自己不自知的）。

```
林尘情感曲线：
  屈辱(9) → 愤怒(8) → 震惊(7) → 坚定(7) → 自信(6) → 恐惧(9) → 坚定(10)
                                                      ↑
                                                 角色弧线完成
```
...
支持 **主线、支线、并行线、闪回线**四种叙事线程，每条线程拥有独立的：
- 视角角色与角色群
- 目标弧线与成长轨迹
- 篇幅权重（自动调整字数分配）
- 掉线预警（超过 5 章未活跃自动提醒）
...
class KnownInfoRecord:
    ...
    info_key: str          # 什么信息
    content: str           # 具体内容
    learned_in_chapter: int    # 在第几章知道的
...
快照备份
    ↓
① 建筑师 Agent ── 规划蓝图（因果链上下文 + 前情摘要 + 伏笔状态 + 跨线程感知）
    ↓
② 写手 Agent ── 生成正文 + 写后结算表（位置/情感/关系/伏笔变化）
    ↓
③ 写后验证器 ── 零 LLM 硬规则检测（字数、禁忌词、格式）
    ↓ error → 现场修正
④ 审计员 Agent ── 叙事质量审计（temperature=0，确保客观一致）
    ↓ critical → 修订者 Agent → 再审（最多 2 轮闭环）
⑤ 因果链提取 ── 从正文中提取因果关系 → 写入世界状态
    ↓
摘要生成 ── 章节摘要注入真相文件
    ↓
状态结算 ── 应用写后结算表 → world_state.json
    ↓
时间轴 + 线程状态更新
```
...
以及 **10 种戏剧功能节拍**：建立、激励事件、转折点、中点、危机、高潮、揭示、决策、后果、过渡。
...
├──────────────────────────────────────────────────┤
│                LLM 抽象层                         │
│   DeepSeek API · Ollama 本地 · OpenAI 兼容      │
└──────────────────────────────────────────────────┘
```
...
```bash
# Windows：双击 `启动服务器.bat` 或 `启动网页界面.bat`（自动打开浏览器）
...
| `df setup init-templates <book>` | 初始化配置模板 |
| `df setup load <book>` | 加载配置到世界状态 |
...
│   ├── METHODS_DOCUMENTATION.md    # 方法论文档
│   └── screenshots/                # 界面截图
...
- [架构设计文档](docs/ARCHITECTURE_DESIGN.md) — 系统架构详解
- [功能设计文档](docs/FUNCTIONAL_DESIGN.md) — 功能设计说明
- [方法论文档](docs/METHODS_DOCUMENTATION.md) — 创作方法论详解
